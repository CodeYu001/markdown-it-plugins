...
function validate(params: string, markup: string): boolean {
   return (
      params
         .trim()
         .split(/\s+/)[0]
...
export default function admonition(md: MarkdownIt): void {
   ...
   md.renderer.rules.admonition_open = (tokens, idx, options, env, self) => {
      ...
      if (type) token.attrJoin('class', `admonition-${type}`);
